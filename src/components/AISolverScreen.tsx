...
export default function AISolverScreen({ userState, onBack, onIncrementSolved }: AISolverScreenProps) {
  ...
  const handleSolve = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "An error occurred during math solver synthesis.");
      }

      setSolutionHTML(data.result);
      onIncrementSolved();
    } catch (err: any) {
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <header className="fixed top-0 w-full z-50 flex items-center justify-between px-6 md:px-16 h-16 bg-[#0f131f]/60 backdrop-blur-xl border-b border-white/10 shadow-[0_0_20px_rgba(0,210,255,0.05)]">
        ...
        <div className="font-sans font-bold text-sm tracking-tight text-slate-400 flex items-center gap-2">
          <span>Target Calibration:</span>
          <span className="text-cyan-400 font-mono px-2 py-0.5 border border-cyan-500/20 bg-cyan-950/20 rounded-full text-xs">
            {currentLevel.name}
          </span>
        </div>
      </header>
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
            ...
            <form onSubmit={handleSolve} className="glass-panel rounded-2xl p-6 border border-slate-800/80 space-y-4">
              <div className="space-y-2">
                <label className="text-xs font-mono text-slate-400 block" htmlFor="problem">
