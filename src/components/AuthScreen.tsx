      ...
      if (isRegister) {
        ...
        if (signUpError) {
          ...
          if (signUpError.message?.includes("already registered") || signUpError.message?.includes("User already exists")) {
            setError("This email is already registered. Please use 'Initialize Sequence' (login) instead.");
            return;
          }
        ...
        // Signup succeeded – redirect directly to dashboard/level-selection
        ...
        if (loginError) {
          ...
          // Show user-friendly login error message
          if (loginError.message?.includes("Invalid login credentials")) {
            setError("Invalid email address or password sequence. Please verify your quantum keys.");
            return;
          }
            ...
            <form onSubmit={handleSubmit} className="space-y-5 relative z-10">
              ...
              <div className="pt-2">
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full cursor-pointer bg-gradient-to-r from-cyan-500 to-cyan-300 text-slate-950 font-bold rounded-lg py-3 px-4 text-xs font-mono tracking-wider flex items-center justify-center gap-2 hover:shadow-[0_0_20px_rgba(0,210,255,0.5)] active:scale-95 transition-all duration-300 uppercase disabled:opacity-70"
                >
                  {loading ? (
